...
function login(wrapper) {
    ...
    <button class="registration__btn" type="submit" data-type='login'>
        Войти
    </button>
    <div class="if-login">
        Ещё нет аккаунта?
...
function renderProfile(wrapper) {
    ...
    function getUser(){
        ...
        .then(res => res.json())
        .then(data => {
          parseJwt(localStorage.getItem(`token`), data)
        })
      }
      function parseJwt (token, dataUsers) {
        ...
        dataUsers.forEach(el => {
          if(JSON.parse(jsonPayload).sub === el.username){
        ...
        if (el.role && roleMappings[el.role]) {
            el.role = roleMappings[el.role];
        }
        ...
        <aside class="sidebar">
            ...
            <label class="sidebar__change-file" style="cursor: pointer; margin-bottom: 18px"  for="user-image">
